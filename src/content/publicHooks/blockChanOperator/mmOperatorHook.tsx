			...
			var completed = function (_contractObj: any, _item: I_contractBlock, _i: string): void {
				...
				if (cCount === totalCount) {
					...
					if (
						...
						contractConfig[projEvn].info.depChainId.toString()
					) {
						setIsWrongNetWork(true);
					} else {
					...
					(function (_item: I_contractBlock, _i: string) {
						...
						item.getAbiAsync(function (_abi: any): void {
							...
							makeOperator(function (_operator) {
								//拉取到合约abi文件后,初始化合约
								let _contractObj: any = new _operator.web3Contract(
									_abi,
									item.id
								);
								//每完成一个合约的初始化,就在这里计数
...
export { useMmOperator as default, useMmOperator as useContractOPerator };
