...
const useWarnBlockChain = function (
    ...
    const dWalletConnected = function (): void {
        if (config.detactWalletConnect) {
            //如果钱包未链接
            if (walletData.currentAccount === '' && walletData.isWalletInstalled && !walletData.isWalletUnInitlized) {
                if (isShow) {
                    setIsShowSilentButton(false);
                    (async function () {
                        if (wcData !== null) {
    ...
    const dWrongNetWork = function (): void {
        if (config.detacWrongNetWork) {
            //如果钱包网络错误
            if (operator.data.isWrongNetWork && walletData.isWalletInstalled && !walletData.isWalletUnInitlized) {
                if (isShow) {
                    setIsShowSilentButton(false);
                    (async function () {
                        if (wnData !== null) {
...
export default useWarnBlockChain;
