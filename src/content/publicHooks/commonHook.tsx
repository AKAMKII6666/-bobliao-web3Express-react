...
const useCommon = function (RUNNING_ENV: string): IcommonContext {
    ...
    const printWarn: ImakeWarn = function ({
        ...
        let warnContents: Array<warnContentsItem> = [
            {
                title: '您當前沒有安裝區塊鏈錢包!',
                content: '安裝區塊鏈錢包后便可進行區塊鏈操作.',
                icon: EWarnIcons.WALLET,
                buttonText: '安裝錢包',
            },
            {
                title: '您已安裝區塊鏈錢包,但錢包并未初始化!',
                content:
                    '您已經完成了錢包的安裝,但是錢包可能並沒有為當前應用初始化,請點擊刷新按鈕嘗試讓錢包在本應用中初始化!',
                icon: EWarnIcons.WALLET,
                buttonText: '刷新应用',
            },
            {
            ...
            {
                title: '網絡選擇錯誤!',
                content: '您當前的區塊鏈錢包所選擇的網絡錯誤，您當前區塊鏈錢包的所選網絡不符合當前應用的需要.',
                icon: EWarnIcons.GLOBAL,
                buttonText: '切換網絡',
            },
            {
                title: '您沒有簽名登錄我們的站點!',
                content: '使用區塊鏈錢包簽名登錄可以解鎖更多功能.',
        ...
        var promise = new Promise<IstateResult>(async function (_res, _rej) {
            var warnContent = (
                <>
                    <Warn title={message.title} content={message.content} icon={message.icon} type={EwarnTypes.SMALL}>
                        {(function () {
    ...
    const translateNumberK = function (_num: number, _fix: number): string {
        ...
        //兆 = 1000000000000
        if (nIARR.length >= 13) {
            var num = new _bigNumber(_num).dividedBy(new _bigNumber(10).exponentiatedBy(12).toFixed()).toFixed(_fix, 1);
            return num + '조';
        }

        //亿 = 100000000
        if (nIARR.length >= 9) {
            var num = new _bigNumber(_num).dividedBy(new _bigNumber(10).exponentiatedBy(8).toFixed()).toFixed(_fix, 1);
            return num + '억';
        }

        //万 = 10000
        if (nIARR.length >= 5) {
            var num = new _bigNumber(_num).dividedBy(new _bigNumber(10).exponentiatedBy(4).toFixed()).toFixed(_fix, 1);
            return num + '만';
        }
    ...
    const translateNumberF = function (_num: number, _fix: number): string {
        ...
        //千万 = 10000000
        if (nIARR.length >= 8) {
            var num = new _bigNumber(_num).dividedBy(new _bigNumber(10).exponentiatedBy(7).toFixed()).toFixed(_fix, 1);
            return num + '千萬';
        }

        //百万 = 1000000
        if (nIARR.length >= 7) {
            var num = new _bigNumber(_num).dividedBy(new _bigNumber(10).exponentiatedBy(6).toFixed()).toFixed(_fix, 1);
            return num + '百萬';
        }
    ...
    const translateNumberT = function (_num: number, _fix: number): string {
        ...
        //t = 10000000000
        if (nIARR.length >= 11) {
            var num = new _bigNumber(_num).dividedBy(new _bigNumber(10).exponentiatedBy(10).toFixed()).toFixed(_fix, 1);
            return num + 'T';
        }

        //b = 1000000000
        if (nIARR.length >= 10) {
            var num = new _bigNumber(_num).dividedBy(new _bigNumber(10).exponentiatedBy(9).toFixed()).toFixed(_fix, 1);
            return num + 'B';
        }
        ...
        //k = 1000
        if (nIARR.length >= 4) {
            var num = new _bigNumber(_num).dividedBy(new _bigNumber(10).exponentiatedBy(3).toFixed()).toFixed(_fix, 1);
            return num + 'K';
        }
        return new _bigNumber(_num).toFixed(_fix);
    };
